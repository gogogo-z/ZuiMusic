 ...
 */
public class AnchorHotAdapter extends RecyclerView.Adapter<AnchorHotAdapter.ViewHolder> implements View.OnClickListener {
    private OnRecyclerViewItemClickListener mOnItemClickListener;

    private Context mContext;

    public List<AnchorRadioBean.RadioList> songSheetList = new ArrayList<>();
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        ...
        viewHolder.tvSongSheetItemListenNum.setText(songSheetList.get(position).getChannelid().substring(0, 5) + "订阅");
        viewHolder.tvSongSheetItemDesc.setText(songSheetList.get(position).getDesc());

        //使用Glide第三方框架加载图片
        Glide.with(mContext)
                .load(songSheetList.get(position).getPic())
    ...
    public interface OnRecyclerViewItemClickListener {
        void onItemClick(View view, AnchorRadioBean.RadioList songSheet);
    }
    ...
    public void onClick(View v) {
        ...
        if (mOnItemClickListener != null) {
            // 注意这里使用getTag方法获取数据
            mOnItemClickListener.onItemClick(v, (AnchorRadioBean.RadioList) v.getTag());
        }
    }

    public void addItems(List<AnchorRadioBean.RadioList> list) {
