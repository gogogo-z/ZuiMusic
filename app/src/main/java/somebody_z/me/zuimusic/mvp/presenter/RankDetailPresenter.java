...
import somebody_z.me.zuimusic.mvp.Contract.RankDetailContract;
import somebody_z.me.zuimusic.mvp.base.BasePresenter;
import somebody_z.me.zuimusic.mvp.model.RankDetailModel;
 ...
 */
public class RankDetailPresenter extends BasePresenter<RankDetailModel, RankDetailFragment> implements RankDetailContract.RankDetailPresenter {
    private String title, url, tag, desc, titleUrl;
    ...
    @Override
    public RankDetailModel loadModel() {
    ...
    public void init() {
        ...
        getiModel().getRankDetail(getIView().subscription, type, offset, size, new RankDetailModel.GetSongSheetDetailListener() {
            @Override
            public void getSuccess(RankDetailBean rankDetailBean) {
                titleUrl = rankDetailBean.getBillboard().getWeb_url();

                title = rankDetailBean.getBillboard().getName();
                collectNum = rankDetailBean.getBillboard().getBillboard_no();
                commentNum = String.valueOf(Integer.valueOf(collectNum) / 6);
                shareNum = String.valueOf(Integer.valueOf(collectNum) / 3);

                url = rankDetailBean.getBillboard().getPic_s210();
                tag = null;
                desc = rankDetailBean.getBillboard().getComment();
                ...
                getIView().setListenNum(collectNum + "0");
                ...
                songSheetDetailList = rankDetailBean.getSong_list();
    ...
    public void addCollectSongSheet() {
        CollectSongSheet collectSongSheet = new CollectSongSheet(type, url, title, tag);
    ...
    public void collectToMySongSheet(final MySongSheet mySongSheet, final ContentBean songInfo) {
        ...
        if (getiModel().searchSongIsCollect(songInfo.getTitle(), getIView().mContext, mySongSheet.getSongSheetName()) == null) {
            ...
            getiModel().getSongInfo(getIView().subscription, songSheetDetailSongID, new RankDetailModel.GetSongInfoListener() {
                @Override
    ...
    public void playSong(ContentBean songSheetDetail) {
        for (int i = 0; i < songSheetDetailList.size(); i++) {
            ...
            getiModel().getSongInfo(getIView().subscription, songid, new RankDetailModel.GetSongInfoListener() {
                @Override
                public void getSuccess(PlaySongInfoBean playSongInfoBean) {
    ...
    public void download() {
        ...
        for (final ContentBean songInfo : songSheetDetailList) {
            getiModel().getSongInfo(getIView().subscription, songInfo.getSong_id(), new RankDetailModel.GetSongInfoListener() {
                @Override
                public void getSuccess(PlaySongInfoBean playSongInfoBean) {
